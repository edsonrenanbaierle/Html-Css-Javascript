...
5- Encerrar e sair do programa;`))
  ...
  switch (decision) {
    ...
    break;
    case 2:
      const nameMember = prompt('Digite o nome do membro que deseja adicionar: ') as string
      const spaceShipe = prompt('Digite o nome da nave no qual quer adicionar o membro: ') as string
      addMember(nameMember, spaceShipe) 
    break;
    case 3:
      const spaceShipeOfMission = prompt('Digite o nome da nave no qual você deseja enviar em missão: ') as string
...
function saveSpaceShip(){
  const spaceShip = {
    name: prompt(`Digite o nome da nave!`) as string,
...
function addMember(name : string, nameShip :string){
...
function sendOfMission(nameShip :string){
  const ship = findRocket(nameShip)
...
function findRocket(name : string){
